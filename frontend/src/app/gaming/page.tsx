...
function Gaming() {
  ...
  const [scores, setScores] = useState<{ s0: number; s1: number }>({
    s0: 0,
    s1: 0
  })
  const [userWallet, setUserWallet] = useState<SolanaTransactionService | null>(
    null
  )
  ...
  useEffect(() => {
    if (isLoggedIn) {
      if (primaryWallet != null) {
        ...
        const _userWallet = new SolanaTransactionService(
  ...
  async function prepare() {
    ...
    if (res0.user == 1) {
      const ress = await userWallet?.Take(
        Number(game_id),
        tokenType as any,
        res0.op
      )
    } else {
      const ress = await userWallet?.createToken(
        200,
        Number(game_id),
        tokenType as any
      )
    ...
    >
      {round < 0 && (
        <div className="w-full flex flex-col items-center">
          ...
          <button
            className={cn(
              'rounded-xl w-full text-2xl shadow px-4 py-2 text-white',
              isPreparing ? 'bg-[#2C2D32]/50' : 'bg-[#2C2D32]'
            )}
            onClick={prepare}
      ...
      {round >= 0 && !isEnd && (
        <div className="w-full">
          <div className="bg-[#2C2D32] drop-shadow-2xl	text-[24px] rounded-[20px] p-4 w-full grid grid-cols-2 grid-flow-col mb-[36px]">
            <div>
              <div>You</div>
          ...
          {quiz && (
            <div className="">
              <div className="text-[32px] p-4 rounded-t-[20px] bg-[#F29300]/90">
              ...
              <div className="flex flex-col gap-y-[16px] text-[24px] glass rounded-b-[20px] p-4">
                <button
                  ...
                  style={{
                    boxShadow:
                      'rgba(0, 0, 0, .1) 0 3px 5px -1px,rgba(0, 0, 0, .14) 0 6px 10px 0,rgba(0, 0, 0, .12) 0 1px 18px 0'
                  }}
                ...
                onClick={async () => {
                  ...
                  if (finalResult) {
                    const res = await userWallet?.CliamRewards(
                      Number(game_id),
                      scores.s0 > scores.s1
                        ? finalResult.user2_token
                        : finalResult.user1_token,
                      scores.s0 > scores.s1
                        ? finalResult.user1_token
                        : finalResult.user2_token,
                      scores.s0 > scores.s1
                        ? finalResult.user2_addr
