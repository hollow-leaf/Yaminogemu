...
export async function matchRegister(address: string, tokenType: string ): Promise<{ "result": boolean, "error": unknown, "match_id": number }> {
...
export async function matchQueue(match_id: number): Promise<{ "result": boolean, "error": unknown, "game_id": number }> {
    const requestOptions = {
        method: "POST",
        headers: {
            'Content-Type': 'application/json', // Ensure the server knows you're sending JSON
